...
# Sony raw_2: 15 evenly spaced test episodes (10,20,...,150), 135-episode train pool.
# The train program further splits the 135-episode pool into 128 train / 7 validation episodes.
...
TASK_NAME="${TASK_NAME:-pick_place_sony_raw2_sp1_sp2_linear_quat_train135_test15_every10_step${TARGET_STEPS}}"
...
TRAIN_RANGES="1-9,11-19,21-29,31-39,41-49,51-59,61-69,71-79,81-89,91-99,101-109,111-119,121-129,131-139,141-149"

[[ -x "$PYTHON_BIN" ]] || { echo "Python 不可用: $PYTHON_BIN" >&2; exit 1; }
[[ -d "$PROJECT_ROOT" ]] || { echo "项目目录不存在: $PROJECT_ROOT" >&2; exit 1; }
...
for path in glob.glob(os.path.join(data_dir, "episode_*.hdf5")):
    match = re.fullmatch(r"episode_(\d+)\.hdf5", os.path.basename(path))
    if match:
        paths[int(match.group(1))] = path
expected = set(range(1, 151))
missing = sorted(expected - set(paths))
if missing:
    raise SystemExit(f"缺少 episode: {missing}")
test_ids = set(range(10, 151, 10))
train_ids = sorted(expected - test_ids)
if len(train_ids) != 135 or len(test_ids) != 15:
    raise SystemExit(f"划分数量错误: train={len(train_ids)} test={len(test_ids)}")
lengths = []
for episode in sorted(expected):
    with h5py.File(paths[episode], "r") as root:
        ...
        if action != qpos or action != state or len(action) != 2 or action[1] != 8:
            raise SystemExit(f"episode {episode}: action={action}, qpos={qpos}, state={state}")
        if images != (action[0], 4, 224, 224):
            raise SystemExit(f"episode {episode}: images={images}")
        ...
        if encoding != "sony_bin_uint16":
            raise SystemExit(f"episode {episode}: image_encoding={encoding!r}")
        if channels != "sp1_high,sp1_low,sp2_high,sp2_low":
            raise SystemExit(f"episode {episode}: channel_names={channels!r}")
        lengths.append(action[0])
inner_train = int(0.95 * len(train_ids))
inner_val = len(train_ids) - inner_train
...
print(max(lengths), len(train_ids), len(test_ids), inner_train, inner_val, batches, target_steps // batches)
PY
)"
read -r EPISODE_LEN TRAIN_COUNT TEST_COUNT INNER_TRAIN INNER_VAL STEPS_PER_EPOCH NUM_EPOCHS <<< "$PREFLIGHT"

echo "Project:             $PROJECT_ROOT"
echo "Dataset:             $DATA_DIR"
echo "Train ranges:        $TRAIN_RANGES"
echo "Test IDs:            $TEST_IDS"
echo "Train pool/test:     $TRAIN_COUNT/$TEST_COUNT"
echo "Internal train/val:  $INNER_TRAIN/$INNER_VAL"
echo "Steps:               $TARGET_STEPS ($STEPS_PER_EPOCH/epoch, $NUM_EPOCHS epochs)"
echo "Checkpoint:          $CHECKPOINT_ROOT/$TASK_NAME"

if [[ "${DRY_RUN:-0}" == "1" ]]; then
    echo "DRY_RUN=1，只完成检查。"
    exit 0
fi
...
cd "$PROJECT_ROOT"
exec "$PYTHON_BIN" train.py --task "$TASK_NAME" --data-format raw10_hdf5 \
