    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None):
        ...
        if self.backbones is not None:
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                features, pos = self.backbones[cam_id](image[:, cam_id])  # changed: backbones[0] -> backbones[cam_id]
...
def build(args):
    state_dim = getattr(args, 'state_dim', 14)  # 双臂 xyz+rpy: 14 (7+7)，默认 14

    # From state
    # backbone = None # from state for now, no need for conv nets
    # From image
    backbones = []
    for _ in args.camera_names:  # changed: add 'for' loop to build multiple backbones
...
def build_cnnmlp(args):
    state_dim = getattr(args, 'state_dim', 14)  # 双臂 xyz+rpy: 14 (7+7)
